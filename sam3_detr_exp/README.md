...
本目录记录项目最早完成的两项核心实验：先把完整SAM3 Base拆成可独立加载和重组的功能模块，再在模块化DETR上进行道路标线LoRA微调。这两项实验本身均已成功；最佳道路标线权重是在后续轻量化阶段才进一步被用作蒸馏教师与效果上限。

所有DETR实验代码、配置、权重、日志和输出均应放在本目录。根目录README只保留总导航和跨实验结论。

第一次复现时建议先看根目录的[最佳实践快速上手](../BEST_PRACTICES.md)，再按本目录文档查看Base
模块化、LoRA实现和消融细节。
...
模块化实验已经完成原始模型与重组模型的图像和视频推理链路验证。实验结论是：SAM3 checkpoint可以成功拆分、独立保存并重新组装，detector、tracker和video predictor链路可用。后续DETR LoRA正是在这套模块化结构上实现的。
