...
本文是PyTorch部署包的实验记录：导出了什么、精度如何、历史上测过多快。
只想知道该用哪个、怎么跑，请看[轻量化部署最佳实践](BEST_PRACTICES.md)；
固定白实线的ONNX/TensorRT/INT8实验见[量化部署记录](QUANTIZATION.md)；
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| E0～E2 | 原始对照、固定文本、合并124处LoRA、FP32单文件 | 已实现，实际238.92 MiB，完整评测见第3节 |
| E3 | 同一图片缓存固定空几何提示编码 | 已实现；保留编码器，7提示约节省20ms，原始输出严格一致 |
| E4 | FP16存储、FP16混合精度与历史BF16计算对照 | 已完成；实际119.60 MiB，平均IoU接近但并非逐位无损 |
| E5 | 预处理、网络前向、后处理及组合链路分项测速 | 已实现，CUDA同步计时，见最佳实践第5节 |
| E6 | 7提示PyTorch批处理、ONNX/TensorRT、AGX实测 | 批处理与固定白实线ONNX/TensorRT已评测；7提示批量导出和AGX仍待执行 |
...
单图推理入口和词表说明见[轻量化部署最佳实践](BEST_PRACTICES.md)第3.2节。
...
这与最佳实践第5节的GPU张量口径不同，两组数字不能相互比较。
...
本轮每卡单模型，和两个模型同卡驻留的计时环境不同，速度比例使用本轮内部比较。
...
2026-09-10测，NVIDIA A800 80GB PCIe，FP32关闭TF32，batch 1，预热30次、重复200次。
每次重新处理已解码的PIL对象；7提示复用图像特征、串行解码，包含PIL转张量、缩放归一化和
mask后处理，不包含模型构建、权重读取/传GPU、磁盘读图、图片解码及文件保存：
...
单提示约8.01张/秒，7提示约2.13张/秒。**这是历史处理器链路速度，不是纯网络或AGX速度**，
已被[最佳实践第5节](BEST_PRACTICES.md)的GPU张量分项口径取代，保留在此仅作过程记录。
压缩后的文件显著变小，但文本查表和LoRA合并只有有限端到端加速；暂不宣称实时部署已经完成。
完整十图输出及正式测速保存于`tests/output/fp32_report.json`。

没有重新训练、裁剪视觉层或更换蛇形卷积；文件压缩主要来自删除MobileCLIP、重复原权重和tracker备份。

## 5. 结论与下一步

- **可用方案**：FP32包为严格同精度参考（238.92 MiB），FP16包为速度/体积候选（119.60 MiB），
  两者都支持全部7个提示。
- **FP16不能称为完全无损**：白实线Recall相对原P12 BF16下降0.00444，超过0.002门槛。
- 固定白实线的TensorRT单提示引擎已实测，端到端24.74ms，见[量化部署记录](QUANTIZATION.md)。

下一步优先在更丰富的图片上复核关键细线Recall，并分别测试FP32权重保留关键层、固定BF16文本
特征等混合方案；空几何常量化仍作为后续独立消融。AGX实测尚未进行。
