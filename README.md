...
- 关于负提示词的来源需要特别澄清：原始SAM3支持负查询与Presence训练，但“每图补齐缺失类别，再随机加入2个`person/dog/cat...`通用词作为纯负提示”来自外部`SAM3_LoRA`项目的自定义训练策略，并非原始SAM3规定的固定做法。历史权重对比显示，单正提示的两版Base LoRA仍能检测`car`，采用高比例多提示负训练后的正式Base则变为0；这提示纯负通用副提示很可能是开放类别能力退化的主因，但仍需严格单变量实验确认。完整证据、假设和待执行实验矩阵见[多提示负训练专项TODO](sam3_detr_exp/docs/multi-prompt-negative-training-todo.md)。
