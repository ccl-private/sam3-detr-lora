...
轻量部署请直接阅读[《SAM3轻量化部署最佳实践》](sam3_lightweight_p12_deploy_exp/BEST_PRACTICES.md)：文档从当前最佳
TinyViT P12出发，说明必需权重、固定文本与LoRA合并、FP32/FP16单文件导出、单提示推理、
预处理/模型前向/后处理分项测速及当前TensorRT、AGX边界，不需要翻阅全部部署消融记录。
...
- 位置：[p10_adaptive_prompt_control](sam3_lightweight_tinyvit_stage3_distill_exp/p10_adaptive_prompt_control/README.md)
- 起点：P9 epoch 19最佳权重；图片恢复原始随机采样，不做整图数量或面积重采样。
- 验证反馈：复用已有验证前向，按0.5阈值累计7类micro IoU、Precision和Recall；训练指标不参与控制。
- 提示控制：只控制正提示是否参与本轮，未选中的可见类别不会被当成负样本；内部负提示始终保留。
- 稳定策略：IoU权重0.7、Recall权重0.3，EMA新值权重0.3，提示率范围0.4～1.0，单轮最大变化0.1。
- 先验：白虚线基础率由目标虚实实例比2.0计算为0.5327，其余类别基础率为1.0。
- 选模：监控macro IoU、macro Recall和最弱类别组成的`val/control_score`，不再仅按验证loss选模。
- 训练：四卡完整完成10轮；白虚线实际提示率由1.0降至0.5624，虚实训练实例比由3.75:1降至约2.17:1。
- 验证：`control_score`最佳为epoch 4的0.7143；但Lightning先完成epoch 0训练再验证，因此epoch 0不是未经训练的纯P9基线。
- 旧10图：验证最佳epoch 4平均IoU为0.6207，最终epoch 9为0.6319；两者均低于P9的0.6439，但epoch 9高于普通P9续训的0.6256。
- 域外3图：epoch 9检出白实线13个、白虚线29个，恢复了P9在城市多车道图中消失的白实线；网络图无真值，只能用于观察跨形态覆盖。
- 结论：提示控制优于普通续训但未超过原始P9；验证集、旧10图和域外图的排序不一致，不能再用单一指标决定总体最佳。

## 8. P11：删除P7高分支并使用新Base教师从头蒸馏

- 位置：[p11_pruned_p7_new_teacher](sam3_lightweight_tinyvit_stage3_distill_exp/p11_pruned_p7_new_teacher/README.md)
- 起点：作者官方TinyViT Stage-3，不继承P9或P10学生权重。
- 单一结构变量：物理删除P7的Stage 1方向特征到`288×288` FPN高分支；保留P5、P6、P7中分辨率分支和P8。
- 教师与训练：复用P9的新Base教师输出缓存，继续关闭域外纯负提示；损失、LoRA覆盖、学习率、batch和20轮训练计划均与P9一致。
- 删除依据：该高分支在P8、P9和P10 epoch 9中的门控绝对值分别为0.0171、0.0174和0.0176，连续三次训练均接近0；P7中分辨率门控则达到1.376。
...
这10张图全部来自无人机视频`DJI_20251231162942_0002_V`的frame 001、005、009至037，
相邻样本仅间隔4帧，并不是10个独立道路场景。标注统计为白实线268个实例、白虚线618个实例，
...
后续使用同样3图、相同提示和0.5阈值补测P9与P10：

| 模型 | 白实线候选 | 白虚线候选 | 人工观察 |
|---|---:|---:|---|
| P8 epoch 4 | 10 | 41 | 覆盖最宽，但存在实线/虚线提示重叠响应 |
| P9 epoch 19最佳 | 8 | 26 | 城市多车道图白实线完全消失，整体更保守 |
| P9 epoch 9中期 | 9 | 28 | 比验证最优点略多，仍未恢复城市图白实线 |
| P10 epoch 9 | **13** | **29** | 恢复城市图白实线，实虚线分离较P8克制 |

P10在旧10图平均IoU仍低于P9，却在3张域外图恢复了P9消失的城市白实线。这不是矛盾，而是说明
旧10图只覆盖单一航拍视频，模型在单场景IoU和跨形态覆盖上的排序可以不同。由于3张网络图没有
人工真值，候选数不能直接解释为精度；后续需要补标mask或建设多视频标注测试集才能定量选模。
