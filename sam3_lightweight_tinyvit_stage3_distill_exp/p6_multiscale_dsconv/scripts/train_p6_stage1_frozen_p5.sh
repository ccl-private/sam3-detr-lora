...
./.venv/bin/python -u \
  sam3_lightweight_tinyvit_stage3_distill_exp/p6_multiscale_dsconv/train_p6_multiscale.py \
  ...
  --student-lora sam3_lightweight_tinyvit_stage3_distill_exp/weights/p5a_dsconv_frozen_20ep.epoch15.pt \
  --checkpoint sam3_lightweight_stage3_exp/input/efficientsam3_tinyvit_stage3.pt \
  --save sam3_lightweight_tinyvit_stage3_distill_exp/weights/p6_stage1_frozen_p5.pt \
  --image-lora-stages 1 2 3 \
  --log-name p6_stage1_frozen_p5 \
  --epochs 10 \
  --batch-size 2 \
  --num-workers 8 \
  --stage1-branch-lr 1e-4 \
  --stage1-gate-lr 1e-3 \
  --stage1-branch-channels 64 \
  --stage1-kernel-size 9 \
  --stage1-offset-scale 1.0 \
  --kd-weight 1.0 \
