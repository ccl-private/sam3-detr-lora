#!/usr/bin/env python3
"""P11：移除P7高分支，使用新Base教师从官方TinyViT起点蒸馏。"""
...
from pruned_structure import P7_BRANCH_PREFIX, attach_pruned_complete_structure
...
class P11PrunedP7Module(P9FreshP8Module):
    """复用P9损失和优化器，只替换细线结构与checkpoint元数据。"""
    ...
    def __init__(
        ...
        P1ImageFeatureDistillModule.__init__(self, *args, student_lora=None, **kwargs)
    ...
    def on_fit_end(self) -> None:
        if self.device.type == "cuda":
            print(
                f"P11单卡峰值显存：allocated={torch.cuda.max_memory_allocated() / 2**30:.2f} GiB，"
    ...
    def on_after_backward(self) -> None:
        groups = {
            ...
            for key in ("detr_lora", "image_lora", "heads", "p5", "p6", "p7_mid", "p8")
        }
        for name, parameter in self.detector.named_parameters():
            ...
            if name.startswith(P5_BRANCH_PREFIX):
                group = "p5"
            elif name.startswith(P6_BRANCH_PREFIX):
                group = "p6"
            elif name.startswith(P7_BRANCH_PREFIX):
                group = "p7_mid"
            elif name.startswith(P8_BRANCH_PREFIX):
                group = "p8"
            elif name.startswith(("dot_prod_scoring.", "segmentation_head.")):
                group = "heads"
        ...
        if self._gradient_reports < 2:
            print(
                "P11梯度检查 step=" + str(self.global_step) + " "
    ...
    def save_lora_checkpoint(self, path: Path) -> None:
        ...
        meta = {
            **self.student_source_meta,
            "experiment": "P11 pruned P7 high branch, fresh TinyViT and new teacher",
            ...
            "p7_highres_fpn": True,
            "p7_high_branch_enabled": False,
            "p7_mid_branch_enabled": True,
...
def main() -> None:
    ...
    parser.add_argument("--log-name", default="p11_pruned_p7_new_teacher")
    parser.add_argument("--save-every-epoch", action="store_true")
    parser.set_defaults(
        data_yaml=P11_DIR / "configs/roadline_no_generic_negatives.yaml",
        cache_root=EXP_DIR / "p9_fresh_p8_new_teacher/cache/new_teacher_outputs",
        save=EXP_DIR / "weights/p11_pruned_p7_new_teacher.pt",
    ...
    if datamodule.train_dataset is None or datamodule.train_dataset.num_generic_negatives != 0:
        raise RuntimeError("P11必须关闭全部域外通用负提示")
    model = P11PrunedP7Module(
    ...
    if hasattr(model.detector.p7_highres_fpn_adapters, "high"):
        raise RuntimeError("P11结构错误：P7 high分支仍然存在")
    trainable = sum(parameter.numel() for parameter in model.parameters() if parameter.requires_grad)
    print(
        f"P11起点=官方TinyViT Stage-3，P7高分支=已删除，域外负提示=0，"
        f"总参数={sum(parameter.numel() for parameter in model.parameters()):,}，"
    ...
    if trainer.is_global_zero:
        ...
        print(f"已保存P11最后权重：{args.save}", flush=True)
