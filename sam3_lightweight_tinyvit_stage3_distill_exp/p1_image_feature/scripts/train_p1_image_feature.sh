...
./.venv/bin/python -u \
  ...
  --student-lora sam3_lightweight_tinyvit_stage3_distill_exp/weights/p0_image_lora_r8.best.pt \
  --checkpoint sam3_lightweight_stage3_exp/input/efficientsam3_tinyvit_stage3.pt \
  --save sam3_lightweight_tinyvit_stage3_distill_exp/weights/p1_image_feature_r8.pt \
  --epochs 10 \
  --batch-size 4 \
  --num-workers 8 \
  --lora-lr 5e-5 \
