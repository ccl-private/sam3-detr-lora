from __future__ import annotations

from pathlib import Path

from sam3.model_builder import build_efficientsam3_image_model
from sam3_detr_exp.utils.detr_lora_utils import (
    attach_lora_to_parametrizable_modules,
    freeze_module,
)

from image_lora import attach_tinyvit_image_lora


EXP_DIR = Path(__file__).resolve().parent
DEFAULT_STAGE3_CHECKPOINT = (
    EXP_DIR.parent
    / "sam3_lightweight_stage3_exp/input/efficientsam3_tinyvit_stage3.pt"
)


def build_trainable_stage3_detector(
    checkpoint_path: Path,
    text_mode: str,
    text_cache_path: Path | None,
    lora_rank: int,
    lora_alpha: float,
    lora_dropout: float,
    decoder_only: bool,
    attn_only: bool,
    train_dot_score: bool,
    train_seg_head: bool,
    image_lora_rank: int = 8,
    image_lora_alpha: float = 16.0,
    image_lora_dropout: float = 0.05,
    image_lora_stages: tuple[int, ...] = (2, 3),
):
    if text_mode != "runtime" or text_cache_path is not None:
        raise ValueError("当前实验仅支持 TinyViT Stage3 的实时 MobileCLIP-S0 文本编码")
    model = build_efficientsam3_image_model(
        checkpoint_path=str(checkpoint_path),
        load_from_HF=False,
        backbone_type="tinyvit",
        model_name="11m",
        text_encoder_type="MobileCLIP-S0",
        text_encoder_context_length=16,
        enable_segmentation=True,
        enable_inst_interactivity=False,
        device="cpu",
    )
    freeze_module(model)
    if train_dot_score:
        for parameter in model.dot_prod_scoring.parameters():
            parameter.requires_grad = True
    if train_seg_head:
        for parameter in model.segmentation_head.parameters():
            parameter.requires_grad = True
    detr_attached = attach_lora_to_parametrizable_modules(
        model=model,
        rank=lora_rank,
        alpha=lora_alpha,
        dropout=lora_dropout,
        include_encoder=not decoder_only,
        include_decoder=True,
        include_ffn=not attn_only,
    )
    image_attached = attach_tinyvit_image_lora(
        model,
        rank=image_lora_rank,
        alpha=image_lora_alpha,
        dropout=image_lora_dropout,
        stages=image_lora_stages,
    )
    model.stage3_lora_metadata = {
        "base_checkpoint": str(checkpoint_path),
        "backbone_type": "tinyvit",
        "model_name": "11m",
        "text_features": {
            "mode": "runtime", "encoder": "MobileCLIP-S0", "context_length": 16,
        },
        "image_lora_stages": list(image_lora_stages),
    }
    return model, detr_attached + image_attached
