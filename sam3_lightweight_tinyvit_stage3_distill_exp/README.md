...
P2之后的结构实验把两类平均IoU从0.4553提高到0.6003：
...
下一步计划建立P8输入侧细线分支：使用`PixelUnshuffle(2)`把RGB无损重排到504分辨率，在大幅下采样前运行低通道DSConv，再通过Stage 2语义门控融合到高分辨率FPN。P8先冻结P7全部旧参数训练5轮；若平均IoU不能超过0.610则停止。代码需预留参数匹配的`1×9 + 9×1`普通长条卷积实现，因为P5-B尚未执行，目前结果不能证明动态蛇形采样本身优于普通长条卷积。
