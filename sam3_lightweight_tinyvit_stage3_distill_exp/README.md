...
¹ 这三组不含蒸馏KD，历史日志没有另写`val/supervised`字段；表中以纯监督`val/loss`作为等价监督损失。P0～P10的`val/supervised`是训练代码直接记录的字段。由于各阶段加入的KD项和历史验证batch聚合口径并不完全相同，`val/loss`绝对值不能跨所有行直接排名；它主要用于同损失配置的相邻实验比较。表中P10相对P9的监督损失和总损失确有小幅下降，但旧10图IoU没有超过P9，正好说明完整验证集训练目标、单视频10图和域外跨形态覆盖不是同一个指标。
