...
./.venv/bin/python -u \
  sam3_lightweight_tinyvit_stage3_distill_exp/p7_highres_fpn/train_p7_highres_fpn.py \
  ...
  --student-lora sam3_lightweight_tinyvit_stage3_distill_exp/weights/p6_stage1_frozen_p5.epoch8.pt \
  --checkpoint sam3_lightweight_stage3_exp/input/efficientsam3_tinyvit_stage3.pt \
  --save sam3_lightweight_tinyvit_stage3_distill_exp/weights/p7_highres_fpn_frozen_p6.pt \
  --image-lora-stages 1 2 3 \
  --log-name p7_highres_fpn_frozen_p6 \
  --epochs 10 \
  --batch-size 2 \
  --num-workers 8 \
  --adapter-lr 1e-4 \
